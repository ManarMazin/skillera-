...
if (isset($_POST['login'])) {
    ...
    $query = "SELECT * FROM manager WHERE man_email = '$email'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($password, $admin['man_pass'])) {
            session_regenerate_id(true); // تجديد معرف الجلسة
            $_SESSION['admin_id'] = $admin['man_id'];
            $_SESSION['admin_email'] = $admin['man_email'];
...
if (isset($_POST['create_account'])) {
    ...
    if ($password !== $confirm) {
        ...
        $check = mysqli_query($connect, "SELECT * FROM manager WHERE man_email = '$email'");
        if (mysqli_num_rows($check) > 0) {
            ...
            $insert = mysqli_query($connect, "INSERT INTO manager (man_name, man_email, man_pass, man_token) VALUES ('$name', '$email', '$hashed_password', '$token')");
