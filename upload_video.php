<?php
session_start();
require 'manar.php';  // ملف الاتصال بقاعدة البيانات

// تأكد من تسجيل دخول المعلم
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($course_id <= 0 || $course_name === '' || $category === '' || $level === '' || $title === '' || $desc === '') {
        $message = "❌ الرجاء ملء جميع الحقول المطلوبة (بما في ذلك Course ID).";
    } else {
        // تحقق من عدم تكرار course_id لنفس المعلم
        $check_query = "SELECT id FROM videos WHERE tea_id = ? AND course_id = ?";
        $check_stmt = mysqli_prepare($connect, $check_query);
        mysqli_stmt_bind_param($check_stmt, 'ii', $tea_id, $course_id);
        ...
        if (mysqli_stmt_num_rows($check_stmt) > 0) {
            ...
            $ins = "INSERT INTO videos 
                (tea_id, course_id, course_name, category, level, title, description, filename, video_url, extra_file, uploaded_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
...
<head>
    ...
    <style>
        ...
        .btn { margin-top: 20px; width: 100%; background: #007BFF; color: #fff; padding: 10px; border: none; border-radius: 6px; cursor: pointer; font-size: 16px; }
...
<button class="btn" onclick="window.location.href='tch_dashboard.php'">الرجوع إلى لوحة تحكم</button>
